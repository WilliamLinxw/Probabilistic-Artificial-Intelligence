    ...
    def train(self):
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                # print("Forward")
                # print("Forward1")
                current_logits, log_prior, log_variational_posterior = self.network.forward(batch_x)
                # print(log_prior.requires_grad)
                # print(log_variational_posterior.requires_grad)
                # print("Loss")
                loss_classification = F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum')
                loss = (1/num_batches)*(log_variational_posterior - log_prior) + loss_classification
                # loss = F.kl_div(log_variational_posterior, log_prior, log_target=True, reduction = 'batchmean') + F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum')
                # loss.backward(retain_graph=True)
                # loss = loss_prior_posterior + loss_classification
                # print("lossed")
                ...
                if batch_idx % self.print_interval == 0:
                    ...
                    current_accuracy = (current_logits.argmax(axis=1) == batch_y).float().mean()
                    progress_bar.set_postfix(loss=loss.item(), acc=current_accuracy.item(), 
                                                log_prior=log_prior.item(), log_posterior= log_variational_posterior.item(),
                                                cla=int(loss_classification))
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.prior = UnivariateGaussian(mu=torch.tensor(0.0), sigma=torch.tensor(0.1))
        ...
        self.weights_var_posterior = MultivariateDiagonalGaussian(
            mu = torch.nn.Parameter(torch.zeros((out_features, in_features)), requires_grad=True),
            rho = torch.nn.Parameter(torch.ones((out_features, in_features))*-10, requires_grad=True)
            )
        ...
        if self.use_bias:
            ...
            self.bias_var_posterior = MultivariateDiagonalGaussian(
                mu = torch.nn.Parameter(torch.zeros((out_features)), requires_grad=True),
                rho = torch.nn.Parameter(torch.ones((out_features))*-10, requires_grad=True)
                )
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        # print(self.weights_var_posterior.mu)
        # print(weights.size())
        # if print_mu_rho <= 10:
        #     print('mu: ', self.weights_var_posterior.mu)
        #     print('rho: ', self.weights_var_posterior.rho)

        log_prior = self.prior.log_likelihood(weights)
        # print(log_prior)
        log_variational_posterior = self.weights_var_posterior.log_likelihood(weights)
        # print("weights posterior: ", log_variational_posterior.item())
        # if print_mu <= 10:
        #     print('mu: ', self.weights_var_posterior.mu)   
        #     print_mu+=1 
        # print(log_variational_posterior.size())
        ...
        if self.use_bias:
            ...
            # print("bias posterior: ", self.bias_var_posterior.log_likelihood(bias).item())
        else:
            bias = None
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for idx, current_layer in enumerate(self.layers):
            # print(idx)
            new_features, log_prior_layer, log_variational_posterior_layer = current_layer.forward(current_features)
            # print(new_features.size(),log_prior_layer.size(), log_variational_posterior_layer.size())
            # if self.print_mu_rho <=10:
            #     # print('print time: ', self.print_mu_rho)
            #     # print('mu: ', current_layer.weights_var_posterior.mu)
            #     self.print_mu_rho += 1
        ...
        # print('posterior: ', log_variational_posterior)
        # print(output_features.size(), log_prior.size(), log_variational_posterior.size())
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        d = values.numel()
        # print(d)
        log_lik = -d*torch.log(self.sigma) - (d*torch.log(torch.tensor(2*np.pi)))/2 - ((values-self.mu)**2).sum()/(2*self.sigma*self.sigma)
        # log_lik = -d/2 * torch.log(2*np.pi*self.sigma**2) - 1/(2*self.sigma**2) * torch.sum(torch.norm(values - self.mu)**2)
        # print("prior mu: ", self.mu)
        # print('prior sigma: ', self.sigma)
        return log_lik
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        # print(self.rho)
        d = values.numel()
        var = F.softplus(self.rho)**2
        # print(var.requires_grad)
        # print('rho size: ', F.softplus(self.rho).size())
        # print('square size: ', torch.square(F.softplus(self.rho)).size())
        # print('square size 2: ', (F.softplus(self.rho)**2).size())

        # print('rho: ', self.rho)
        # print('var:', var)
        
        log_lik = -(d*torch.log(torch.tensor(2*np.pi))) - (torch.log(torch.prod(var)+1e-6))/2 - (((values-self.mu)*(1/(var+1e-6))*(values-self.mu)).sum())/2
        # print(log_lik.requires_grad)
        # print('posterior: ', log_lik)
        # log_lik = -.5 * (2 * torch.sum(torch.log(sigma)) + torch.sum(torch.norm((values - self.mu)/sigma)**2) + d * torch.log(torch.tensor(2 * np.pi)))
        # loss = torch.nn.GaussianNLLLoss(full=True, reduction='sum')
        # print(log_lik)
        # print(-loss( values, self.mu, var ))
        # # print(log_lik==-loss( values, self.mu, var ))
        # return -loss( values, self.mu, var )
    ...
    def sample(self) -> torch.Tensor:
        ...
        sample = self.standard_normal.sample(self.mu.size()) * sigma + self.mu
        return sample
